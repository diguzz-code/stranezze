...
try {
    ...
    1..1000 | ForEach-Object {
        if (-not $ready) {
            try {
                Invoke-WebRequest -Uri 'http://127.0.0.1:8099/api' -UseBasicParsing | Out-Null
                $ready = $true
            } catch {
    ...
    $created = Invoke-RestMethod -Uri 'http://127.0.0.1:8099/api' -Method Post -ContentType 'application/json' -Body $payload
    if (-not $created.item) { throw 'Creazione fallita' }
    $items = Invoke-RestMethod -Uri 'http://127.0.0.1:8099/api'
    if ($items.items.Count -lt 1) { throw 'Elenco vuoto dopo la creazione' }
    $found = Invoke-RestMethod -Uri 'http://127.0.0.1:8099/api?q=%3Cscript%3E'
